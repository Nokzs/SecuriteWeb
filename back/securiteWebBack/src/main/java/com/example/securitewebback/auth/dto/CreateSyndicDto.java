...
public record CreateSyndicDto(
                @NotBlank @Size(min = 10, max = 50) @Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).*$", message = "Le mot de passe doit contenir au moins un chiffre, une minuscule, une majuscule et un caractère spécial") String password,
