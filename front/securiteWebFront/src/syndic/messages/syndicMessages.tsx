...
export function SyndicMessages() {
  ...
  const { data, isLoading, refetch } = useQuery<Page<Message>>({
    ...
    queryFn: async () => {
      const res = await secureFetch(
        `${API_BASE}/syndics/messages?limit=${filter.limit}&page=${filter.page}&type=${filterType}`,
      );
  ...
  const archiveMutation = useMutation({
    mutationFn: async (messageId: string) => {
      await secureFetch(`${API_BASE}/syndics/messages/${messageId}/archive`, {
        method: "PATCH",
      });
    },
    onSuccess: () => {
          ...
          {data?.content.map((msg) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${msg.isRead ? "bg-slate-50 text-slate-400" : "bg-indigo-100 text-indigo-600"}`}
                >
                  <Mail size={16} />
                </div>
                <div className="flex-1 min-w-0">
                  <h3
                    className={`font-bold truncate ${msg.isRead ? "text-slate-500" : "text-slate-800"}`}
                  >
        ...
        <div className="lg:col-span-7">
          {selectedMessage ? (
            <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden sticky top-6 shadow-sm">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 bg-slate-800 rounded-xl flex items-center justify-center text-white font-bold text-lg">
                      {selectedMessage.senderFirstName[0]}
                      {selectedMessage.senderLastName[0]}
                    </div>
                    <div>
                      <h2 className="text-xl font-bold text-slate-900 leading-tight">
                        {selectedMessage.senderFirstName}{" "}
                        {selectedMessage.senderLastName}
                      </h2>
                      <p className="text-sm text-slate-500 font-medium">
                        {selectedMessage.senderEmail || "Pas d'email"}
                      </p>
                    </div>
                  </div>
              ...
              <div className="p-8">
                <h4 className="text-[10px] uppercase font-black text-slate-300 tracking-widest mb-6">
                  Contenu du message
                </h4>
